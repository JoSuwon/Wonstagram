...
<%
	...
	feedArr = feedDAO.searchFeed(column, val);
...
<script>
	...
	$(function() {
		if(feedList.length > 0) {
			...
			$('#wrap').append('<h2 style="text-align:center; margin-top: 30px;">검색되는 피드가 없습니다.</h2>');
			endFeed = true;			
		}
		...
		window.addEventListener('scroll', function() {
			...
			if(scrollLocation + windowHeight >= fullHeight*0.97){
				if(feedList.length <= index && !endFeed) {
					$('#wrap').append('<h2 style="text-align:center; margin-top: 30px;">더이상 검색된 피드가 없습니다.</h2>');
					endFeed = true;
